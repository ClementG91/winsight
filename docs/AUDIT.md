...
`259056b` (RA-02 à RA-05, WS-74 à WS-78) : **la tête de la branche n'est pas qualifiée**.
...
- **Problèmes trouvés.** L'audit a traité **72 défauts** : 67 corrigés et testés (dont quatre dans le
...
| WS-78 | Low | Validation | La vérification du stockage des VM ne connaissait que l'identité propre à chaque VM (`S-1-5-83-1-*`) : elle refusait l'accès en écriture qu'Hyper-V accorde aussi à la capacité de son processus de travail (`vmWorkerProcess`, `S-1-15-3-1024-…`), vu au premier lancement de la protection sur le disque de données de la VM de contrôle | protection du stockage impossible, donc aucune passe VM ; rien de modifié au-delà du propriétaire des disques et du verrou du dossier parent | `WinSightHyperV.psm1` | cette capacité, et elle seule, acceptée dans le stockage des VM seulement : une capacité ne compte que pour un jeton AppContainer, au second de ses deux contrôles d'accès, et ne donne donc jamais l'écriture à un utilisateur local ; SID recalculé comme Windows le fait (SHA-256 du nom en majuscules) et vérifié par `DeriveCapabilitySidsFromName` ; test de contrat et quatre autocontrôles PowerShell 5.1, en échec sur l'ancien module | Corrigé |
...
| Suite de tests complète | ~5 min, 3 606 tests (2 976 au début de l'audit), 0 échec | Release, 23 projets |
...
complète (3 606 tests, 0 échec), le build Release (0 avertissement, avertissements traités comme
...
| Kit de qualification VM | `docs/validation/VM_QUALIFICATION_KIT.md`, `VmQualificationKitContractTests.cs`, `scripts/Measure-CloudFilesAccess.ps1`, `ScriptParameterDefaultContractTests.cs` | WS-71, WS-72, WS-77, WS-78, WS-40 (mesure) |
