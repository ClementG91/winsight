...
- **Firewall (Phase 2)**, **WFP** (Windows Filtering Platform) user-mode filters
  keyed by app id. A user-mode WFP filter cannot hold a connection while a user decides, so an unknown application is recorded as pending after it connects; a prompt-driven default-deny mode is planned in [`OBJECTIVE_SEE_IMPLEMENTATION_PLAN.md`](OBJECTIVE_SEE_IMPLEMENTATION_PLAN.md) (M4). WFP alone (no driver) covers
