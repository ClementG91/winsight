...
| **x64, audit branch** | **Qualified as a local rehearsal (2026-09-25/26).** The rebuilt harness (RA-01) qualified the head's product code, with verified provenance for each run (`docs/AUDIT.md` §17.4): the full run `head-fe953fe` (32 gates, 0 failures), gate 17 again with the corrected Cloud Files measurement (`gate17-711ded0`), and gate 36 from the control VM (`net-711ded0b`, 10/10). The product code (`src`, `installer`) is the same in those candidates and in the head; later commits changed only the harness, tests and documentation. Local, unsigned builds: not CI-attested, not signed. Commit hashes cited before 2026-09-24 predate a history rewrite that removed local paths; `docs/AUDIT.md` §17.3 maps them to the published ones. |
