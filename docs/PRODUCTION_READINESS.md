...
| **x64, audit branch** | **Current head not qualified.** A local, unsigned candidate of the branch, `259056b`, passed every x64 functional VM gate on 2026-09-23 (below), with a provenance caveat. The branch has changed product code since (RA-02 to RA-05 and WS-74 to WS-76, from `bb608ff`), so the affected gates must run again on a new candidate once the qualification harness is hardened (RA-01). Not CI-attested, not signed. |
