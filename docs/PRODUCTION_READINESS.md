...
| **x64, audit branch** | **Current head not qualified.** A local, unsigned candidate of the branch, `259056b`, passed every x64 functional VM gate on 2026-09-23 (below), with a provenance caveat. The branch has changed product code since (RA-02 to RA-05 and WS-74 to WS-76, from `bb608ff`; WS-77 to WS-83 fixed the harness itself). The rebuilt harness (RA-01) ran a full qualification on `fe953fe` with verified provenance: 32 gates, 0 failures (`docs/AUDIT.md` §17.4). Gate 36 (network logon, a separate two-VM run) and gate 17 (Cloud Files, with a probe that now fails each download request at once so that every read is attributed to its process) must still pass on the next candidate. Not CI-attested, not signed. Commit hashes cited before 2026-09-24 predate a history rewrite that removed local paths; `docs/AUDIT.md` §17.3 maps them to the published ones. |
