...
| **x64, audit branch** | **Current head not qualified.** A local, unsigned candidate of the branch, `259056b`, passed every x64 functional VM gate on 2026-09-23 (below), with a provenance caveat. The branch has changed product code since (RA-02 to RA-05 and WS-74 to WS-76, from `bb608ff`; WS-77 to WS-83 fixed the harness itself). The first run of the rebuilt harness (RA-01) passed 30 of 32 gates on `771a67b` with verified provenance (`docs/AUDIT.md` §17.4); gate 17 (Cloud Files, now attributing each download request to its process) and gate 36 (network logon) must run again on the next candidate. Not CI-attested, not signed. Commit hashes cited before 2026-09-24 predate a history rewrite that removed local paths; `docs/AUDIT.md` §17.3 maps them to the published ones. |
